#!/usr/bin/env python3
"""
增强版：视频与轨迹数据对齐脚本（支持逐帧OCR验证）

改进：
1. 使用OCR从最后一帧识别实际时间戳
2. 根据视频时长反推开始时间
3. 新增：逐帧OCR验证模式，对每一帧进行OCR识别确认时间
4. 自动检测和矫正时间偏移
"""
...
class VideoTrajectoryAlignerEnhanced:
    ...
    def __init__(self, trajectory_path: str, video_dir: str, output_dir: str,
        ...
        Args:
            ...
            verbose: 是否输出详细信息
            frame_by_frame_ocr: 是否对每一帧进行OCR验证
        """
        ...
        self.frame_by_frame_ocr = frame_by_frame_ocr
    ...
    def extract_frames_per_second(self, video_info: Dict) -> List[Dict]:
        ...
        如果启用了 frame_by_frame_ocr，会对每一帧进行OCR验证
        """
        aligned_frames = []
        ...
        # 确定处理模式
        if self.frame_by_frame_ocr:
            iterator = tqdm(range(int(duration)), desc=f"  处理 {video_info['filename']}")
        else:
            iterator = range(int(duration))
        ...
        for second in iterator:
            ...
            # 如果启用了逐帧OCR验证
            if self.frame_by_frame_ocr:
                ocr_time = self.extract_timestamp_from_frame(frame)
                if ocr_time:
                    # 使用OCR识别的时间
                    frame_time = ocr_time
    ...
    def process_all_videos(self) -> pd.DataFrame:
        ...
        for video_info in video_info_list:
            if video_info['end_time'] < overlap_info['trajectory_start']:
                if self.verbose and not self.frame_by_frame_ocr:
    ...
    def save_results(self, aligned_df: pd.DataFrame, overlap_info: Dict):
        ...
        stats = {
            'summary': {
                ...
                'frame_by_frame_ocr': self.frame_by_frame_ocr
            },
        ...
        if self.verbose:
            ...
            if self.frame_by_frame_ocr:
                print(f"✓ 使用了逐帧OCR验证模式")


def main():
    parser = argparse.ArgumentParser(description='视频与轨迹数据对齐工具（增强版 - 支持逐帧OCR）')
    ...
    parser.add_argument('--time-tolerance',
                       ...
                       default=2,
                       help='时间容差（秒）')
    parser.add_argument('--frame-by-frame-ocr',
                       action='store_true',
                       help='对每一帧进行OCR验证（更准确但更慢）')
    ...
    aligner = VideoTrajectoryAlignerEnhanced(
        trajectory_path=args.trajectory,
        video_dir=args.video_dir,
        output_dir=args.output,
        time_tolerance=args.time_tolerance,
        verbose=not args.quiet,
        frame_by_frame_ocr=args.frame_by_frame_ocr
    )
